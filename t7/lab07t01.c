...
int main(int argc, char** argv){
	...
	if ((childPid = fork())){
		...
		fprintf(stderr, "Parinte: PID= %d, PPID= %d PidCOPIL= %d.\n", getpid(), getppid(), childPid);

		signal(SIGUSR2, (void (*))SIGUSR2_handler);

		fprintf(stderr, "Parinte: PID= %d. Urmeaza un apel pause() si astept SIGUSR2 de la copil.\n", getpid());

		ret = pause();
		errNO = errno;

		fprintf(stderr, "Parinte: PID= %d. PPID= %d -- SIGUSR2 livrat de copil PID= %d m-a scos din pause() . Am iesit cu errno=%d.\n", getpid(), getppid(), childPid, errNO);

		if (kill(childPid, SIGINT) < 0){
			perror("kill(): ");
		}

		fprintf(stderr, "Parinte: PID= %d, PPID= %d am emis SIGINT (ctrl-c) catre PidCOPIL= %d.\n", getpid(), getppid(), childPid);
		...
		fprintf(stderr, "Copil: PID= %d, PPID= %d. Emit SIGUSR2 (= %d) catre parinte.\n", getpid(), getppid(), SIGUSR2);
